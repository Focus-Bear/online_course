...
const LessonItem = ({ lesson, position }: any) => {
  ...
  return (
    <div className='w-full h-fit flex flex-col gap-2 relative pr-4'>
      <div className='absolute top-1.5 text-lg right-6 w-fit h-fit text-white'>
        {isCollapseOpened ? (
          <MdKeyboardArrowUp />
        ) : (
          <MdKeyboardArrowDown />
      ...
      <div
        ...
        className='w-full h-fit text-sm font-semibold tracking-wider cursor-pointer rounded px-2 py-1 bg-sky-900 hover:bg-sky-800'
      ...
      <UnmountClosed isOpened={isCollapseOpened}>
        <div className='w-full h-[10%] flex items-center bg-gray-200 gap-2 px-2 py-1 rounded-t-md relative'>
          <div className='font-bold text-sm text-blue-700'>{`Title`}</div>
          <input
        ...
        <div className='w-full h-[15%] flex items-center justify-between bg-gray-200 px-2 py-2 rounded-b-md relative'>
          <div className='w-[15%] font-bold text-sm text-blue-700'>{`Youtube URL`}</div>
          <input
